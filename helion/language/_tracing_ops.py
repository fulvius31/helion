...
@_decorators.codegen(_mask_to, "openmp")
def _(state: CodegenState) -> ast.AST:
    # CPU slicing auto-clamps at tensor boundaries, so masking is typically
    # unnecessary.  If a mask_var exists (non-power-of-2 block size), apply it
    # with torch.where; otherwise pass through unchanged.
    tensor = state.proxy_arg(0)
    assert isinstance(tensor, torch.Tensor)
    other = state.proxy_arg(1)
    assert isinstance(other, (int, float, bool))
